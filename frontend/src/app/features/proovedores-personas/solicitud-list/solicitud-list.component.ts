...
import { SolicitudServicio, ServicioOption, EventoOption, CatalogoOption } from '../../proveedores/proveedor.models';
...
export class SolicitudListComponent implements OnInit {
    ...
    protected onEdit(solicitud: any): void {
        this.router.navigate(['/proveedores-personas', 'solicitudes', solicitud.id, 'editar']);
    }
    ...
    protected createSolicitud(): void {
        this.router.navigate(['/proveedores-personas', 'solicitudes', 'nuevo']);
    }
    ...
    private loadCatalogos(): void {
        ...
        this.proveedorService.getAllPersonas({ page: 0, size: 200 }).subscribe({
            next: (response) => {
                ...
                // Cargar nombres desde getPersonas en paralelo para tener los nombres reales
                this.proveedorService.getPersonas().subscribe(personas => {
                    const mapPersonas = new Map(personas.map(p => [p.id, `${p.nombres} ${p.apellidos}`]));
                    response.content.forEach((p) => {
                        const nombre = p.personaId ? mapPersonas.get(p.personaId) : (p.especialidad || 'Proveedor');
                        this.proveedoresMap.set(p.id, nombre || 'Proveedor');
                    });
                    checkReady();
                });
            },
